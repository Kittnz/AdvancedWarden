...
{
	TC_LOG_WARN("warden", "Send module to client");
...
{
	TC_LOG_WARN("warden", "Request module");
    ...
    if (_initialized)
    {
        ...
        if (_dataSent)
        {
            ...
            if (maxClientResponseDelay > 0)
            {
                ...
                {
					TC_LOG_WARN("WARDEN", "Player %s(guid: %u, account : %u, latency : %u, IP : %s) exceeded Warden module response delay for more than %s - disconnecting client");
